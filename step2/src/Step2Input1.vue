            ...
            <div class="mx-3 my-5">
                ...
                >
                    ...
                    <div>
                        ...
                        >
                            <template v-slot:activator="{ on, attrs }">
                                <v-text-field
                                    v-model.lazy="formData.onsetDate"
                                    label="発症日"
                    ...
                    <v-text-field label="住所"
                            v-model.lazy="formData.attendantAddress"
                            >
                    </v-text-field>
                    <v-text-field label="電話番号"
                            v-model.lazy="formData.attendantPhone"
                            :rules="phoneRules"
                            ref="attendantPhone"
                            >
                    </v-text-field>
                    ...
                    <v-card
                        v-for="(x, i) in activityList"
                        :key="i"
                        class="px-2 my-3"
                    >
                    <v-card-title class="workplace-header">
                        活動{{1+i}}
                    </v-card-title>
                        <v-text-field
                            label="場所名"
                        ...
                        <v-select label="その活動に合うと思う特徴"
                                v-model="x.profile"
                                :items="activityProfileList"
                                placeholder="リストから選択してください。"
                                >
                        </v-select>
                        <v-text-field
                            label="活動の内容を教えて下さい。"
                            v-model="x.detail"
                            placeholder="例）医療事務、レジ打ち、ホールでの接客、ピアノ教室"
                        ></v-text-field>
                        <v-card-actions>
                            <v-spacer/>
                            <v-btn
...
export default {
    data() {
        return {
            valid: false,
            activePicker: null, // onset, birthday 共通
            menuOnset: false,
            menuBirthday: false,
            nationsList: utils.nationsList,
            sexList: [ '回答しない','男性','女性','その他'],
            residence: '',
            prefecture: '',
            city: '',
            activityFeatureList: [
    ...
    computed: {
        ...
        prefectures: function() {
            return Object.keys(lgList)
        },
        cities: function() {
            return lgList[this.prefecture]
        },
    },
    watch: {
        // date_picker
    ...
    methods: {
        prefectureSelected: function() {
            this.formData.livingArea = ''
        },
        finishResidence: function() {
            this.formData.livingArea = `${this.prefecture}${this.city}`
        },
        ...
        // date_picker
        saveOnset: function(content) {
            this.$refs.onsetPicker.save(content)
        },
        saveBirthday: function(content) {
            this.$refs.birthdayPicker.save(content)
        },
    },
    mounted: function() {
